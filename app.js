...
function noticeBox(n, price, live){ // 注意/處置風險（持股卡用，完整，仿處置股 App）
  if(!n) return "";
  ...
  const approx = n.approx
    ? (live
        ? `<div class="nrow nsub">＊即時估算版：用大盤指數(TAIEX)近似全體市場，非每日批次的全市場真實均值，明天完整更新後會更準</div>`
        : `<div class="nrow nsub">＊非虧損股，未納入同類條件，估算偏保守</div>`)
    : "";
